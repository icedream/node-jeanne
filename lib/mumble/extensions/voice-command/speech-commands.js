...
const commands = {
	...
	move_forward: {
		detector: /(vor(spring|spul)e?n?|(spul|spring)e?\s+vor)/i,
		func: function (matches, tr) {
	...
	move_backward: {
		detector: /(zurück(spring|spul)e?n?|(spul|spring)e?\s+zurück)/i,
		func: function (matches, tr) {
			const player = this.space.get('youtube-player')
			player.jumpBackward()
		}
	...
	afk: {
		detector: /\b(afk|away from keyboard|abwesenheitskanal|abwesenheits-?channel)\b/i,
		func: function (matches, tr) {
			this.invoke('afk')
		}
	}
